...
<div class="upper_div">
		...
		<div class = "inner_div">
				<a href="ta_projects.php" class="links" style="background-color: rgb(230,230,230);">Проекты</a>
		</div>
		<div class = "inner_div">
...
<div class="help">
<div class="div1">
<p class="text">Создание проекта:<br><hr style="border-color: rgb(204,191,158);"></p>
<form method="post">
	<p class="text">Введите название проекта:<input type="text" style="width: 10em;" class="vvod" name="project"autocomplete="off"></p>
 <p class="text"><input type="submit" value="Создать" class ="submit" /></p>
</form>
<hr style="border:1px dashed; border-color: rgb(204,191,158);" ">
<p class="text">Удаление проекта:<br><hr style="border-color: rgb(204,191,158);"></p>
<form method="post">
	<p class="text">Введите название проекта:<input type="text" style="width: 10em;" class="vvod" name="del_project" autocomplete="off"></p>
...
if(isset($_POST['project']) & $_POST['project']!="")
{
	...
	$query = "SELECT name FROM `ta_bp`.`projects` WHERE name = '".$_POST['project']."';";
	$res = mysqli_query($conn,$query);
if($res){
$ch = true;
while($row = mysqli_fetch_array($res))
{
	if($row['name'] = $_POST['project'])
	{

		echo "Такой проект уже существует".": ".$row['name'];
		$ch = false;
...
$sql = "INSERT INTO `ta_bp`.`projects` (name)
...
if(isset($_POST['del_project']) & $_POST['del_project']!="")
{

	$query = "DELETE FROM `ta_bp`.`projects` WHERE name = '".$_POST['del_project']."';";
	$res = mysqli_query($conn,$query);
...
if($role == 'admin'){
	$query1 = "SELECT * FROM `ta_bp`.`projects`";
...
<td style="border-bottom-width: 2px;">{$row1['name']}</td>
