...
<div class="upper_div">
		...
		<div class = "inner_div">
				<a href="ta_users.php" class="links" style="background-color: rgb(230,230,230);">Редактирование<br>пользователей</a>
		</div>
		<div class = "inner_div">
		...
		<div class = "inner_div">
				<a href="ta_doc_code.php" class="links">Коды документов</a>
		</div>
...
<div class="help">
<div class="div1">
<p class="text">Создание пользователя:<br><hr style="border-color: rgb(204,191,158);"></p>
<form method="post">
	<p class="text">Введите логин пользователя:<input type="text" style="width: 10em;" class="vvod" name="login"autocomplete="off"></p>
	<p class="text">Задайте пароль:<input type="password" style="width: 10em;" class="vvod" name="password"autocomplete="off"></p>
	<p class="text">Введите ФИО пользователя:<input type="text" style="width: 10em;" class="vvod" name="full_name"autocomplete="off"></p>
	<p class="text">Выберите роль:
	<select name="role" class="selection">
...
<p class="text">Удаление пользователя:<br><hr style="border-color: rgb(204,191,158);"></p>
<form method="post">
	<p class="text">Введите логин пользователя:<input type="text" style="width: 10em;" class="vvod" name="del_login" autocomplete="off"></p>
	...
	isset($_POST['full_name']) & $_POST['full_name']!="")
{
	...
	$query = "SELECT login FROM `users`.`users` WHERE login = '".$_POST['login']."';";
...
while($row = mysqli_fetch_array($res))
{
	if($row['login'] = $_POST['login'])
	{
		...
		echo "Такой пользователь существует".": ".$row['login'];
		$ch = false;
...
$sql = "INSERT INTO `users`.`users` (login,password,full_name,role)
	VALUES ('".$_POST['login']."','".$_POST['password']."','".$_POST['full_name']."',
	'".$_POST['role']."');";
...
if(isset($_POST['del_login']) & $_POST['del_login']!="")
{

	$query = "DELETE FROM `users`.`users` WHERE login = '".$_POST['del_login']."';";
	$res = mysqli_query($conn,$query);
...
if($role == 'admin'){
	$query1 = "SELECT * FROM `users`.`users`";
...
<td style="border-bottom-width: 2px;">{$row1['login']}</td>
<td style="border-bottom-width: 2px;">{$row1['password']}</td>
<td style="border-bottom-width: 2px;">{$row1['full_name']}</td>
<td style="border-bottom-width: 2px;">{$row1['role']}</td>
</tr>
END;
/*echo "ID: ".$row['id']."<br>\n";
